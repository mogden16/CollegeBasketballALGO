...
-------------------------------------------
Workflow:
  1. Refresh kenpom_raw.txt and barttorvik_raw.txt manually or with scripts/refresh_ratings.py
  2. Run this script to generate today's predictions + Discord alerts
  3. Use slate_results.py for results entry, checking, and reports
...
def parse_barttorvik(filepath: str) -> dict[str, Team]:
    """
    Parse BartTorvik T-Rank data from either:
      1. The original staggered copy-paste format from barttorvik.com/trank.php, or
      2. A normalized TSV export generated by scripts/refresh_ratings.py.
    """
    teams = {}
    with open(filepath, "r") as f:
        lines = [line.rstrip("\n") for line in f]
    ...
    normalized_header = {part.lower(): idx for idx, part in enumerate(header_parts)}
    if {"team", "adjoe", "adjde"}.issubset(normalized_header):
        tempo_idx = next(
            (normalized_header[key] for key in ("adj t.", "adj t", "adjt") if key in normalized_header),
            None,
        )
        if tempo_idx is None:
            return teams

        required_idx = max(normalized_header["team"], normalized_header["adjoe"], normalized_header["adjde"], tempo_idx)
        for line in lines[1:]:
            parts = [part.strip() for part in line.split("\t")]
            if len(parts) <= required_idx:
                continue

            team_name = parts[normalized_header["team"]]
            if not team_name:
                continue

            try:
                adj_o = float(parts[normalized_header["adjoe"]])
                adj_d = float(parts[normalized_header["adjde"]])
                adj_t = float(parts[tempo_idx])
            except ValueError:
                continue
